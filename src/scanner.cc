/*
 * Unless flagged by a CUSTOM comment, all code is copied from
...
#include <cstring>
// CUSTOM
#include <../node_modules/tree-sitter-html/src/tag.h>
...
// CUSTOM just the name
void *tree_sitter_php_embedded_external_scanner_create() {
  return new Scanner();
}

// CUSTOM just the name
bool tree_sitter_php_embedded_external_scanner_scan(void *payload, TSLexer *lexer,
...
// CUSTOM just the name
unsigned tree_sitter_php_embedded_external_scanner_serialize(void *payload, char *buffer) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  return scanner->serialize(buffer);
}

// CUSTOM just the name
void tree_sitter_php_embedded_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  scanner->deserialize(buffer, length);
}

// CUSTOM just the name
void tree_sitter_php_embedded_external_scanner_destroy(void *payload) {
  Scanner *scanner = static_cast<Scanner *>(payload);
  delete scanner;
}
